                       ...
                       context:(void *)context
{
    ...
    if ([keyPath isEqualToString:NSStringFromSelector(@selector(maximumNumberOfLogFiles))])
    {
        ...
        __weak DDLogFileManagerDefault *weakSelf = self;
        dispatch_async([DDLog loggingQueue], ^{ @autoreleasepool {
            
            [weakSelf deleteOldLogFiles];
        }});
    }
}
...
- (void)setMaximumFileSize:(unsigned long long)newMaximumFileSize
{
    ...
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        [weakSelf maybeRollLogFileDueToSize];
...
- (DDLogFileInfo *)currentLogFileInfo
{
    if (currentLogFileInfo == nil)
    {
        ...
        if ([sortedLogFileInfos count] > 0)
        {
            ...
            if (!_doNotReuseLogFiles && !mostRecentLogFileInfo.isArchived)
            {
...
- (void)addExtendedAttributeWithName:(NSString *)attrName
{
    ...
    if (result < 0)
    {
        NSLogError(@"DDLogFileInfo: setxattr(%s, %s): error = %s",
                   path,
                   name,
                   strerror(errno));
    }
...
- (void)removeExtendedAttributeWithName:(NSString *)attrName
{
    ...
    {
        NSLogError(@"DDLogFileInfo: removexattr(%s, %s): error = %s",
                   path,
                   name,
                   strerror(errno));
    }
